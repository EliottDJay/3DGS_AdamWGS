...
then
  ...
  python $ROOT/train.py \
      --config=$abspath"/config.yaml" --seed $seed --distributed $distributed 2>&1 | tee start_log/3dgs_$now.txt
else
  echo "distributed, but not implemented yet"
  exit 1
  python $ROOT/train.py \
      --config=$abspath"/config.yaml" --seed $seed --distributed $distributed --master_addr $3  -master_port   2>&1 | tee start_log/3dgs_$now.txt
fi