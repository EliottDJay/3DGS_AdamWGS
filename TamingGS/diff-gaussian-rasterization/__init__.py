...
# TODO: 
class SparseGaussianAdam2(torch.optim.Adam):
    def __init__(self, params, lr, eps, opt):
        super().__init__(params=params, lr=lr, eps=eps)

        self.img_panelty = 0
        self.scaling_penalty_begin = opt.scaling_penalty_begin
        self.scaling_penalty_end = opt.scaling_penalty_end
        self.opacity_penalty_begin = opt.opacity_penalty_begin
        self.opacity_penalty_end = opt.opacity_penalty_end

        print(f"scaling_penalty: {self.scaling_penalty_begin} - {self.scaling_penalty_end}", file=sys.stderr)
        print(f"opacity_penalty: {self.opacity_penalty_begin} - {self.opacity_penalty_end}", file=sys.stderr)
        
        self.asystep = None

    def _img_scale(self, img_pixel=None):
        if img_pixel is None:
            self.img_panelty = 100000  
        else:
            assert isNum(img_pixel), "The img_pixel should be a number, but got {}".format(type(img_pixel))
            num_str = str(img_pixel)
            length = len(num_str)
            first_digit = int(num_str[0])
            self.img_panelty = first_digit * (10 ** (length - 2))
    
    @torch.no_grad()
    def step(self, visibility, N, iteration):

        if self.asystep is None:
            self.asystep = torch.zeros_like(visibility, dtype=torch.int32)
        self.asystep.add_(visibility)
        ...
        for group in self.param_groups:
            ...
            reg = 0
            if "opacity" in group['name'] and iteration >= self.opacity_penalty_begin and iteration <= self.opacity_penalty_end:
                reg = 1
            elif "scaling" in group['name'] and iteration >= self.scaling_penalty_begin and iteration <= self.scaling_penalty_end:
                reg = 2
            ...
            exp_avg_sq = stored_state["exp_avg_sq"]
            M = param.numel() // N
            _C.adamUpdate2(param, param.grad, exp_avg, exp_avg_sq, self.asystep, visibility, lr, 0.9, 0.999, eps, reg, self.img_panelty, N, M)