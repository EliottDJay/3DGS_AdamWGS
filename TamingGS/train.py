...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, websockets, score_coefficients, args):
    ...
    gaussians = GaussianModel(dataset.sh_degree, opt.optimizer_type, opt=opt)
    ...
    # TODO: use_random
    use_er = opt.use_er
    er_begin = opt.er_begin
    er_end = opt.er_end
    er_interval = opt.er_interval
    if use_er:
        er_strategy = opt.er_strategy
        print("Re State Reg, begin: {}, end: {}, with {} strategy".format(er_begin, er_end, er_strategy), file=sys.stderr)

    # TODO: load noise transport
    noise_transport = opt.noise_transport
    noise_begin = opt.noise_begin
    noise_end = opt.noise_end
    noise_lr = opt.noise_lr
    print("noise_transport: {}, from {} to {}, with lr {}".format(noise_transport, noise_begin, noise_end, noise_lr), file=sys.stderr)
    #exit(1)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background), args.model_path)
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    v_filter = gaussians.densify_with_score(scores = gaussian_importance, 
                ...
                if iteration % opt.opacity_reset_interval == 0 or (dataset.white_background and iteration == opt.densify_from_iter):
                    ...
                    print("begining reset_opacity", file=sys.stderr)
            ...
            if iteration < opt.iterations:
                if opt.optimizer_type == "default":
                    ...
                    gaussians.optimizer.zero_grad(set_to_none = True)
                elif opt.optimizer_type == "sparse_ours":
                    visible = radii > 0
                    #if iteration > 598:
                        #print(f"radii shape: {visible.shape}, asystep shape {gaussians.optimizer.asystep.shape}", file=sys.stderr)
                    #print(f"visibility_filter, {v_filter.shape}; {v_filter.bool().shape}", file=sys.stderr)
                    gaussians.optimizer.step(v_filter.bool(), v_filter.shape[0], iteration)
                    gaussians.optimizer.zero_grad(set_to_none = True)
            ...
            # TODO: 
            if use_er and (iteration >= er_begin) and (iteration <= er_end) and (iteration % er_interval == 0):
                gaussians.restate_regularization(iteration)
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, filepath):
    if tb_writer:
    ...
    if iteration in testing_iterations or iteration == 30000:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                with open(os.path.join(filepath, 'log_file.txt'), 'a') as file:
